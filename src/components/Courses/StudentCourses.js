...
const StudentCourses = () => {
  ...
  function getStudentRegisteredCourseForDashboard() {
    axios.get(`http://localhost:8080/api/v1/student/courses-registered?studentId=${userPayload.userPayload.studentId}`)
      .then(response => {
        setStudentCourses(response.data.payload);
      })
      .catch(() => {
      });
  }
  ...
  useEffect(() => {
    if (userPayload?.userPayload?.studentId) {
      getStudentRegisteredCourseForDashboard();
      axios.get(`http://localhost:8080/api/v1/student/course-options?studentId=${userPayload.userPayload.studentId}`)
        .then(response => {
  ...
  }, [userPayload?.userPayload?.studentId]);
  ...
  const handleEnrollCourse = () => {
    axios.post(`http://localhost:8080/api/v1/student/register-course?courseId=${courseEnrollmentData.courseId}&studentId=${userPayload?.userPayload?.studentId}`)
      .then(() => {
        getStudentRegisteredCourseForDashboard();
      })
      .catch(() => {});
  ...
  return (
    <>
              ...
              <div className="col-xxl-6 col-xl-6 col-lg-6 col-md-8">
                <div className="section__title-wrapper mb-45">
                  <div className="d-flex justify-content-between">
                    ...
                    <button className="e-btn" onClick={registerCourse}>Register Course</button>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="category__item mb-30 transition-3 d-flex align-items-center">
                    ...
                    <div className="category__content">
                      <h4 className="category__title">
                        <Link href={`/student-course-details?courseId=${item.courseId}`}>
                          <a>{item.courseName}</a>
                        </Link>
                      </h4>
                      <p>{item.courseCode}</p>
      ...
      >
        ...
        <DialogContent>
          <TextField
            value={courseEnrollmentData.courseId}
            onChange={(e) => {
              setCourseEnrollmentData({courseId: e.target.value})
            }}
            id="courseToEnroll"
            label="Select Course"
            fullWidth
          ...
          >
            {courses.map((course, index) => {
              return <MenuItem key={index} value={course.courseId}>{course.courseName}</MenuItem>
            })}
          </TextField>
        </DialogContent>
        <DialogActions>
          <button className="e-btn" onClick={() => {
            setModalVisibility(false);
            setCourseEnrollmentData({courseId: ''})
          }}>Cancel
          </button>
          <button className="e-btn" onClick={handleEnrollCourse}>Save</button>
        </DialogActions>
      </Dialog>
    </>
  );
